...
export default function ListeScreen({ navigation }) {
	...
	const max = employes.length
		? Math.max(...employes.map((e) => Number(e.salaire)))
		: 0;
	const min = employes.length
		? Math.min(...employes.map((e) => Number(e.salaire)))
		: 0;

	// Rendu d'une carte employé
	const renderItem = ({ item, index }) => {
		const obs =
			item.obs && OBS_COLORS[item.obs] ? item.obs : getObs(item.salaire);
				...
				<View style={{ flex: 1 }}>
					<View style={styles.row}>
						...
						<Text style={styles.salaire}>
							{Number(item.salaire).toLocaleString()} €
						</Text>
					</View>
					<View style={styles.row}>
						...
						>
							<Text style={[styles.badgeText, { color: cl.text }]}>
								{obs.toUpperCase()}
							</Text>
						</View>
					</View>
		...
		<SafeAreaView style={styles.safe}>
			...
			<View style={styles.header}>
				...
				<View style={styles.headerRight}>
					<TouchableOpacity style={styles.iconBtn}>
						<Icon name="settings" size="medium" color={THEME.textMid} />
					</TouchableOpacity>
					<TouchableOpacity style={styles.iconBtn}>
						<Icon name="notifications" size="medium" color={THEME.textMid} />
					</TouchableOpacity>
				</View>
			</View>
			...
			<View style={styles.statsRow}>
				{[
					...
					{
						label: "Grands",
						value: employes.filter(
							(e) =>
								(e.obs && OBS_COLORS[e.obs] ? e.obs : getObs(e.salaire)) ===
								"grand",
						).length,
						color: "#2E7D32",
					},
