...
const OBS_COLORS = {
  grand:    { text: '#2E7D32', bg: '#E8F5E9', border: '#A5D6A7', chart: '#4CAF50' },
  moyen:    { text: '#E65100', bg: '#FFF3E0', border: '#FFCC80', chart: '#FF9800' },
  mediocre: { text: '#C62828', bg: '#FFEBEE', border: '#EF9A9A', chart: '#F44336' },
};
...
export default function StatsScreen({ navigation }) {
  ...
  const grandC    = employes.filter(e => ((e.obs && OBS_COLORS[e.obs]) ? e.obs : calcObs(e.salaire)) === 'grand').length;
  const moyenC    = employes.filter(e => ((e.obs && OBS_COLORS[e.obs]) ? e.obs : calcObs(e.salaire)) === 'moyen').length;
  const mediocreC = employes.filter(e => ((e.obs && OBS_COLORS[e.obs]) ? e.obs : calcObs(e.salaire)) === 'mediocre').length;
  ...
  const pieData = [
    { name: 'Grand',    population: grandC,    color: '#4CAF50', legendFontColor: THEME.textDark, legendFontSize: 12 },
    { name: 'Moyen',    population: moyenC,    color: '#FF9800', legendFontColor: THEME.textDark, legendFontSize: 12 },
    { name: 'Médiocre', population: mediocreC, color: '#F44336', legendFontColor: THEME.textDark, legendFontSize: 12 },
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.kpiGrid}>
          {[
            { label: 'Total salarial', value: `${total.toLocaleString('fr-FR')} €`, color: THEME.primary,  bg: '#E8F3E0', icon: '💼' },
            { label: 'Salaire moyen',  value: `${moyenne.toLocaleString('fr-FR')} €`, color: '#E65100',   bg: '#FFF3E0', icon: '⚖️' },
            { label: 'Salaire max',    value: `${max.toLocaleString('fr-FR')} €`,     color: '#2E7D32',   bg: '#E8F5E9', icon: '📈' },
            { label: 'Salaire min',    value: `${min.toLocaleString('fr-FR')} €`,     color: '#C62828',   bg: '#FFEBEE', icon: '📉' },
          ].map((k, i) => (
        ...
        <View style={styles.repartitionCard}>
          <Text style={styles.cardSectionTitle}>Répartition par observation</Text>
          <View style={styles.repartitionRow}>
            {[
              ...
              const pct = employes.length ? Math.round((r.count / employes.length) * 100) : 0;
              return (
                <View key={r.obs} style={[styles.repartItem, { backgroundColor: cl.bg, borderColor: cl.border }]}>
                  <Text style={[styles.repartCount, { color: cl.text }]}>{r.count}</Text>
                  <Text style={[styles.repartLabel, { color: cl.text }]}>{r.label}</Text>
                  <Text style={[styles.repartPct, { color: cl.text + 'AA' }]}>{pct}%</Text>
                </View>
              );
            })}
          </View>
        ...
        <View style={styles.toggle}>
          {[
            ...
            <TouchableOpacity key={t.key}
              style={[styles.toggleBtn, chartType === t.key && styles.toggleBtnActive]}
              onPress={() => setChartType(t.key)}
              activeOpacity={0.75}
            >
              <Text style={[styles.toggleText, chartType === t.key && styles.toggleTextActive]}>
                {t.label}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        <View style={styles.chartCard}>
          <Text style={styles.chartTitle}>
            {chartType === 'histogram' ? 'Salaires par employé' : 'Répartition par observation'}
          </Text>
          {chartType === 'histogram' ? (
            <BarChart
              data={barData}
        ...
        <View style={styles.tableCard}>
          ...
          <View style={[styles.tableRow, styles.tableHeader]}>
            <Text style={[styles.tableCell, styles.tableHeaderText, { flex: 2 }]}>Employé</Text>
            <Text style={[styles.tableCell, styles.tableHeaderText]}>N°</Text>
            <Text style={[styles.tableCell, styles.tableHeaderText, { textAlign: 'right' }]}>Salaire</Text>
            <Text style={[styles.tableCell, styles.tableHeaderText, { textAlign: 'right' }]}>Obs.</Text>
          </View>
          {employes.map((emp, i) => {
            const obs = (emp.obs && OBS_COLORS[emp.obs]) ? emp.obs : calcObs(emp.salaire);
            const cl = OBS_COLORS[obs];
            return (
              <View key={emp.id} style={[styles.tableRow, i % 2 === 0 && styles.tableRowAlt]}>
                <View style={{ flex: 2, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  <View style={[styles.tableAvatar, { backgroundColor: cl.bg, borderColor: cl.border }]}>
                    <Text style={[styles.tableAvatarText, { color: cl.text }]}>
                      {emp.nom.charAt(0).toUpperCase()}
                    </Text>
                  </View>
                  <Text style={styles.recapNom} numberOfLines={1}>{emp.nom}</Text>
                </View>
                <Text style={styles.recapNum}>{emp.numemp || emp.numEmp}</Text>
                <Text style={[styles.recapSal, { color: cl.text, textAlign: 'right' }]}>
                  {Number(emp.salaire).toLocaleString()}
                </Text>
                <View style={[styles.recapBadge, { backgroundColor: cl.bg, borderColor: cl.border, alignSelf: 'center' }]}>
...
const styles = StyleSheet.create({
  ...
  headerTitle:      { fontSize: 28, fontWeight: '900', color: THEME.textDark, letterSpacing: -0.5 },
  ...
  cardSectionTitle: { fontSize: 13, fontWeight: '800', color: THEME.textDark, marginBottom: 12 },
  ...
  chartTitle:       {
    color: THEME.textMid, fontSize: 13, fontWeight: '700',
  ...
  tableHeader:      { backgroundColor: THEME.bg, borderBottomWidth: 2, borderBottomColor: THEME.border },
  tableCell:        { flex: 1 },
  tableHeaderText:  { fontSize: 10, fontWeight: '800', color: THEME.textLight, textTransform: 'uppercase', letterSpacing: 0.5 },
  tableAvatar:      {
  ...
  recapBadge:       { borderRadius: 6, borderWidth: 1, paddingHorizontal: 6, paddingVertical: 2 },
